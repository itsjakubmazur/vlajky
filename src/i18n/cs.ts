...
export const cs = {
  ...
  party: {
    title: 'Turnaj',
    desc: 'Hrajte na jednom zařízení proti sobě. Každý dostane stejné otázky.',
    open: 'Turnaj',
    playersTitle: 'Kdo hraje',
    playerCount: 'Kolik vás je',
    playerName: (i: number) => `Hráč ${i}`,
    namePlaceholder: 'Jméno',
    modeTitle: 'Co se bude hrát',
    lengthTitle: 'Kolik otázek v kole',
    start: 'Začít turnaj',
    handoffTitle: 'Předej zařízení',
    handoffWho: (name: string) => `Hraje ${name}`,
    handoffHint: 'Ostatní se prosím nedívají.',
    handoffStart: 'Jdu na to',
    roundLabel: (n: number) => `${n}. kolo`,
    roundResult: 'Výsledek kola',
    winner: (name: string) => `Kolo vyhrává ${name}`,
    winnerTie: 'Kolo skončilo remízou',
    standings: 'Pořadí turnaje',
    wins: (n: number) => (n === 1 ? '1 kolo' : n < 5 ? `${n} kola` : `${n} kol`),
    noWins: 'zatím nic',
    nextRound: 'Další kolo',
    finish: 'Ukončit turnaj',
    finishConfirm: 'Opravdu ukončit? Výsledky se smažou.',
    resume: 'Pokračovat v turnaji',
    resumeHint: (played: number) => (played === 0 ? 'Rozehraný turnaj' : `Odehráno ${played} kol`),
    notCounted: 'Turnaj se nepočítá do tvého postupu ani do hodnosti.',
    waiting: (name: string) => `Čeká: ${name}`,
    done: 'Dohráno',
  },
  daily: {
    title: 'Denní výzva',
